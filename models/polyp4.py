"""Static SAM2 + two-class BiomedCLIP dense prompting for polyp4."""
...
from .static_sam2 import StaticSAM2DensePrompt


POLYP4_POSITIVE_TEXT = "a colonoscopy image showing a colorectal polyp"
POLYP4_NEGATIVE_TEXT = "a normal colonoscopy image showing healthy colon tissue"
POLYP4_TEXTS = (POLYP4_POSITIVE_TEXT, POLYP4_NEGATIVE_TEXT)

POLYP4_SAM_LORA_RANKS = (128, 256, 512)
POLYP4_CLIP_LORA_RANKS = (128, 256)
# Rank 128 is retained only for loading the completed legacy polyp4 run.
POLYP4_TEXT_LORA_RANKS = (64, 128)


def resolve_polyp4_ranks(
    ...
    if sam not in POLYP4_SAM_LORA_RANKS:
        raise ValueError(f"SAM2 Image Encoder rank must be one of {POLYP4_SAM_LORA_RANKS}, got {sam}")
    if clip not in POLYP4_CLIP_LORA_RANKS:
        raise ValueError(f"BiomedCLIP visual rank must be one of {POLYP4_CLIP_LORA_RANKS}, got {clip}")
    if text not in POLYP4_TEXT_LORA_RANKS:
        raise ValueError(f"BiomedCLIP text rank must be one of {POLYP4_TEXT_LORA_RANKS}, got {text}")
    return sam, clip, text


def polyp4_rank_label(sam_lora_rank: int, clip_lora_rank: int, text_lora_rank: int) -> str:
    sam, _, _ = resolve_polyp4_ranks(sam_lora_rank=sam_lora_rank)
    _, clip, _ = resolve_polyp4_ranks(clip_lora_rank=clip_lora_rank)
    _, _, text = resolve_polyp4_ranks(text_lora_rank=text_lora_rank)
...
class Polyp4Model(nn.Module):
    """Static SAM2 model with 512-D CLIP alignment and 256-D dense prompting."""

    SUPPORTED_LORA_RANKS = (128,)
    SUPPORTED_SAM_LORA_RANKS = POLYP4_SAM_LORA_RANKS
    SUPPORTED_CLIP_LORA_RANKS = POLYP4_CLIP_LORA_RANKS
    SUPPORTED_TEXT_LORA_RANKS = POLYP4_TEXT_LORA_RANKS
    IMAGE_SIZE = 352
    CLIP_GRID_SIZE = 22
    ...
    def __init__(
        ...
        super().__init__()
        self.sam_lora_rank, self.clip_lora_rank, self.text_lora_rank = resolve_polyp4_ranks(
        ...
        self.register_buffer(
            "polyp4_prompt_tokens",
            self.biomedclip.tokenizer(list(POLYP4_TEXTS)),
        )

        self.prompt_adapter = ClipDensePromptAdapter(grid_size=self.CLIP_GRID_SIZE)
        self.static_sam2 = StaticSAM2DensePrompt(self.sam2)
    ...
    def forward(self, sam_images: torch.Tensor, clip_images: torch.Tensor):
        ...
        text_features_512 = self.biomedclip.encode_text(
            self.polyp4_prompt_tokens,
            normalize=True,
        )
        logit_scale = self.biomedclip.model.logit_scale.exp().detach()
        similarity_logits = patch_text_similarity(
            patch_tokens_512,
            text_features_512,
            scale=logit_scale,
        ...
        outputs = self.static_sam2(sam_images, dense_prompt)
        return {
            **outputs,
...
__all__ = [
    ...
    "POLYP4_TEXTS",
    "POLYP4_SAM_LORA_RANKS",
    "POLYP4_CLIP_LORA_RANKS",
    "POLYP4_TEXT_LORA_RANKS",
    "resolve_polyp4_ranks",
    "polyp4_rank_label",
